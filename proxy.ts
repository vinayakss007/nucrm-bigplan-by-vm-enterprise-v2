...
/**
 * Advertised on every /api/ response so clients can detect the API generation
 * they are talking to (#52). Bump this when the API contract changes shape.
 */
...
/**
 * Headers attached to API responses that are built by hand (429/401/403/500),
 * which bypass setCORS. `x-request-id` is generated by the proxy when the
 * caller does not supply one.
 */
function apiErrorHeaders(requestId: string, extra: Record<string, string> = {}): Record<string, string> {
  return { 'x-request-id': requestId, 'X-API-Version': API_VERSION, ...extra };
}

function setCORS(response: NextResponse, origin: string | null, pathname: string): void {
  if (!pathname.startsWith('/api/')) return;

  // Every /api/ response passes through here, which makes this the one place
  // the version header has to be set for the pass-through paths.
...
function buildRateLimitResponse(requestId: string, result: { allowed: boolean; remaining: number; reset: number; limit: number }, origin: string | null): NextResponse {
  const headers: Record<string, string> = apiErrorHeaders(requestId, getRateLimitHeaders(result));
  if (origin) {
...
export async function proxy(request: NextRequest) {
  ...
  if (!JWT_SECRET) {
    ...
    return new NextResponse(JSON.stringify({ error: 'Server configuration error' }), {
      status: 500,
      headers: apiErrorHeaders(requestId, { 'content-type': 'application/json' }),
    });
  }
  ...
  if (!token) {
    if (isApiRequest(pathname)) {
      return NextResponse.json({ error: 'Authentication required' }, { status: 401, headers: apiErrorHeaders(requestId) });
    }
  ...
  try {
    ...
    if (isApiRequest(pathname)) {
      ...
      if (needsCsrfValidation(request.method, pathname, authMethod)) {
        ...
        if (!validateCsrfToken(cookieCsrf, headerCsrf)) {
          return NextResponse.json({ error: 'CSRF token missing or invalid. Please refresh the page and try again.' }, { status: 403, headers: apiErrorHeaders(requestId) });
        }
      }
    ...
    if (isApiRequest(pathname)) {
      return NextResponse.json({ error: 'Invalid or expired token' }, { status: 401, headers: apiErrorHeaders(requestId) });
    }
