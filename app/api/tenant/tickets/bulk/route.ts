    ...
    switch (action) {
      case 'assign': {
        ...
        await db.transaction(async (tx) => {
          ...
          logAudit({ tenantId: ctx.tenantId, userId: ctx.userId, action: 'bulk_assign', entityType: 'ticket', entityId: validIds[0], newData: { count: validIds.length, user_id: userId }, dbOrTx: tx as any });
        });
        break;
      }
      case 'status': {
        ...
        await db.transaction(async (tx) => {
          ...
          affected = validIds.length;
          logAudit({ tenantId: ctx.tenantId, userId: ctx.userId, action: 'bulk_status', entityType: 'ticket', entityId: validIds[0], newData: { count: validIds.length, status }, dbOrTx: tx as any });
        });
        break;
      }
      case 'priority': {
        ...
        await db.transaction(async (tx) => {
          ...
          affected = validIds.length;
          logAudit({ tenantId: ctx.tenantId, userId: ctx.userId, action: 'bulk_priority', entityType: 'ticket', entityId: validIds[0], newData: { count: validIds.length, priority }, dbOrTx: tx as any });
        });
        break;
      }
      case 'delete': {
        await db.transaction(async (tx) => {
          ...
          affected = validIds.length;
          logAudit({ tenantId: ctx.tenantId, userId: ctx.userId, action: 'bulk_delete', entityType: 'ticket', entityId: validIds[0], newData: { count: validIds.length }, dbOrTx: tx as any });
        });
        break;
      }
      default:
