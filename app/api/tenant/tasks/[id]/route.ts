...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    const [row] = await withConcurrencyGuard(
      () => {
        ...
        if (clientVersion) {
          whereConditions.push(updatedAtMs(tasks, existing.updatedAt!));
        }
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    void logError({ error: err, context: 'tenant/tasks/[id] PATCH' });
    return apiError(err); 
  }
});
...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
    ...
    void logError({ error: err, context: 'tenant/tasks/[id] DELETE' });
    return apiError(err); 
  }
});
