...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isValid) {
      void logError({ error: new Error('PayU hash verification failed'), context: 'webhook-payu:hash-verification', level: 'warning', metadata: { txnid } });
    ...
    if (!UUID_RE.test(quoteId)) {
      void logError({ error: new Error('PayU could not derive a valid quote id'), context: 'webhook-payu:derive-quote-id', level: 'warning', metadata: { txnid } });
    ...
    try {
      result = await db.transaction(async (tx): Promise<ProcessResult> => {
        ...
        if (status === 'success') {
          ...
          if (!Number.isFinite(paidAmount) || paidAmount <= 0) {
            void logError({ error: new Error(`PayU invalid amount '${amount}'`), context: 'webhook-payu:invalid-amount', level: 'warning', metadata: { txnid, amount } });
          ...
          if (invoice) {
            if (invoice.status === 'cancelled' || invoice.status === 'void') {
              void logError({
                error: new Error(`PayU refusing to record payment against ${invoice.status} invoice`),
                context: 'webhook-payu:invoice-not-payable',
                level: 'warning',
                metadata: { txnid, invoiceId: invoice.id, invoiceStatus: invoice.status },
              });
      ...
      void logError({ error: dbErr, context: 'webhook-payu:db-update', metadata: { txnid } });
    ...
    void logError({ error: err, context: 'webhook-payu:process-callback' });
