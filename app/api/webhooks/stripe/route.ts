...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    void logError({ error: err, context: 'webhooks/stripe signature verification', level: 'warning' });
...
async function handlePaymentFailed(invoice: any) {
  ...
  if (tenant) {
    ...
    }).catch((e) => void logError({ error: e, context: 'webhooks/stripe payment-failed admin telegram', level: 'warning' }));
  }
}
