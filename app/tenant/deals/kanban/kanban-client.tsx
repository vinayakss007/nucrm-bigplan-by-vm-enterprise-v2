...
const StageColumn = memo(function StageColumn({ stage, deals, _onDealMove }: { stage: Stage; deals: Deal[]; _onDealMove: (dealId: string, newStageId: string) => void }) {
...
export default function DealsKanbanPage() {
  ...
  const dealsByStage = useMemo(() => {
    ...
    for (const s of stages) map[s.id] = deals.filter(d => d.stageId === s.id);
    return map;
  }, [deals, stages]);
  const totalValue = useMemo(() => deals.reduce((sum, d) => sum + (parseFloat(d.amount || '0') || 0), 0), [deals]);
  const wonDeals = useMemo(() => deals.filter(d => d.stageId === 'won'), [deals]);
  const wonValue = useMemo(() => wonDeals.reduce((sum, d) => sum + (parseFloat(d.amount || '0') || 0), 0), [wonDeals]);
    ...
    <div className="space-y-4 sm:space-y-6 animate-fade-in">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <Link href="/tenant/deals" className="flex items-center gap-1.5 px-3 py-2 bg-violet-600 text-white rounded-lg hover:bg-violet-700 text-xs sm:text-sm shrink-0">
