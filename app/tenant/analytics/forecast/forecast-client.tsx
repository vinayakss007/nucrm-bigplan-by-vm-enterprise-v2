...
export default function ForecastPage() {
  ...
  const monthlyData = useMemo((): MonthlyForecast[] =>
    next6Months.map(ym => {
  ...
  const totalPipeline = useMemo(() => deals.reduce((s, d) => s + parseFloat(String(d.amount || 0)), 0), [deals]);
  ...
  const avgDealSize = useMemo(() => (dealCount > 0 ? totalPipeline / dealCount : 0), [totalPipeline, dealCount]);
        ...
        {chartData.some(d => d.pipeline > 0 || d.weighted > 0) ? (
          <ResponsiveContainer width="100%" height={280}>
            <BarChart data={chartData}>
              ...
              <Tooltip formatter={(val: TooltipValueType | undefined) => formatCurrency(Number(val ?? 0))} />
