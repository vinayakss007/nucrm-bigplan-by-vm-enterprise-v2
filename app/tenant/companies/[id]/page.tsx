...
export default async function CompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  let ctx;
  try { ctx = await requireTenantCtx(); } catch { redirect('/auth/login'); }
  ...
  `);
  const counts = (result as { rows?: Array<Record<string, number>> }).rows?.[0] ?? { contact_count: 0, lead_count: 0, deal_count: 0, pipeline_value: 0 };
