...
export default async function CompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  `);
  const countsRows = (result as unknown as { rows?: Array<{ contact_count: number; lead_count: number; deal_count: number; pipeline_value: number }> }).rows;
  const counts = countsRows?.[0] ?? { contact_count: 0, lead_count: 0, deal_count: 0, pipeline_value: 0 };
