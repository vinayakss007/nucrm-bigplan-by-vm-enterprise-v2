...
const COLUMNS: Column[] = [
  ...
  { id: 'deferred', name: 'Deferred', color: 'bg-purple-400' },
...
const TaskCard = memo(function TaskCard({ task, isDragging }: { task: Task; isDragging?: boolean }) {
...
const TaskColumn = memo(function TaskColumn({ column, tasks }: { column: Column; tasks: Task[] }) {
  return (
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {COLUMNS.map(col => (
          <div key={col.id} className="admin-card p-3">
            ...
            <p className="text-lg font-bold mt-1">{tasksByStatus[col.id]?.length || 0}</p>
          </div>
        ))}
      </div>
