...
const TicketCard = memo(function TicketCard({ ticket, isDragging }: { ticket: Ticket; isDragging?: boolean }) {
...
const TicketColumn = memo(function TicketColumn({ column, tickets }: { column: Column; tickets: Ticket[] }) {
  return (
...
export default function TicketsKanbanPage() {
  ...
  const ticketsByStatus = useMemo(() => {
    ...
    for (const col of COLUMNS) map[col.id] = tickets.filter(t => t.status === col.id);
    return map;
  }, [tickets]);
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {COLUMNS.map(col => (
          <div key={col.id} className="admin-card p-3">
            ...
            <p className="text-lg font-bold mt-1">{ticketsByStatus[col.id]?.length || 0}</p>
          </div>
        ))}
      </div>
