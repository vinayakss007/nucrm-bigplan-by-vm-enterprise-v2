            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                ...
                <div key={key}>
                  ...
                  <input type="number" value={f[key as keyof typeof f]} onChange={e=>setF(p=>({...p,[key as string]:Number(e.target.value)}))} className={inp}/>
                </div>
              ))}
            </div>
          </div>
...
export default function BillingPage() {
  ...
  const [tenants, setTenants] = useState<Array<{ plan_id: string; status: string }>>([]);
  ...
  return (
    <div className="space-y-5 max-w-6xl">
      {(editPlan || creating) && <PlanForm plan={editPlan ?? undefined} onSave={load} onClose={()=>{setEditPlan(null);setCreating(false);}}/>}
