...
export default function AuditError({ error, reset }: { error: Error; reset: () => void }) {
  useCaptureError(error, 'superadmin-audit');
  return (
    <div className="flex items-center justify-center h-64 p-8">
      <div className="text-center max-w-sm">
        <AlertTriangle className="w-10 h-10 text-red-500 mx-auto mb-3" />
        <h2 className="font-semibold text-white mb-1">Audit Log Error</h2>
        <p className="text-sm text-white/40 mb-4">Failed to load audit log data.</p>
