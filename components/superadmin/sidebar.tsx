    ...
    return (
      <aside className="w-[3.5rem] shrink-0 h-full flex flex-col items-center py-2 gap-0.5 transition-all duration-200 overflow-y-auto bg-card border-r border-border">
        ...
        <Link href="/tenant/dashboard" title="My CRM"
          className="w-9 h-9 flex items-center justify-center rounded-lg mt-auto mb-1 text-violet-400 hover:bg-violet-500/10 transition-colors">
    ...
    <aside className="w-[15rem] shrink-0 h-full flex flex-col transition-all duration-200 bg-card border-r border-border">
      ...
      <div className="h-14 flex items-center justify-between px-4 shrink-0 border-b border-border">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[0.5625rem] font-bold px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-950/30 text-amber-600 dark:text-amber-400">ADMIN</span>
        </div>
      ...
      <Link href="/tenant/dashboard"
        className="flex items-center gap-2 mx-3 mt-3 px-3 py-2 rounded-lg text-xs font-medium transition-colors bg-violet-500/10 text-violet-400 hover:bg-violet-500/20 shrink-0">
        ...
        {filtered.map(section => {
          ...
          return (
            <div key={section.id} className="space-y-0.5">
              ...
              >
                <span className="text-[0.5625rem] font-bold uppercase tracking-widest text-muted-foreground/40 group-hover:text-muted-foreground/70 transition-colors">
                  {section.label}
                </span>
                {!q && (
              ...
              {isOpen && section.items.map(item => {
                ...
                return (
                  <Link key={item.href} href={item.href}
                    className={cn('flex items-center gap-2.5 px-3 py-2 rounded-lg text-[0.8125rem] font-semibold transition-all',
      ...
      <div className="p-2.5 shrink-0 border-t border-border">
        ...
        <div className="flex items-center gap-2.5 px-3 py-2">
          ...
          <div className="min-w-0">
            ...
            <p className="text-[0.625rem] truncate text-muted-foreground">{profile?.email}</p>
          </div>
        </div>
      </div>
    </aside>
