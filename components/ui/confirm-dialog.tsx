...
export async function confirmThen(
  ...
  return new Promise<boolean>((resolve) => {
    ...
    const escapedLabel = confirmLabel.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    ...
    card.innerHTML = `
      <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1rem">
        ...
        <div>
          ...
          <p style="font-size:0.875rem;color:hsl(var(--muted-foreground));margin:0.25rem 0 0;line-height:1.5">${escapedMessage}</p>
        </div>
      </div>
      <div style="display:flex;justify-content:flex-end;gap:0.5rem">
        ...
        <button data-action="confirm" style="padding:0.5rem 1rem;font-size:0.875rem;font-weight:500;border-radius:0.5rem;border:none;background:hsl(var(--destructive));color:hsl(var(--destructive-foreground));cursor:pointer">${escapedLabel}</button>
      </div>
    `;
