...
vi.mock('@/drizzle/schema/chat', () => ({
  chatSessions: { id: 'id', tenantId: 'tenant_id', status: 'status', visitorId: 'visitor_id', assignedTo: 'assigned_to', convertedLeadId: 'converted_lead_id' },
  chatMessages: { id: 'id', sessionId: 'session_id', tenantId: 'tenant_id', createdAt: 'created_at' },
}));

vi.mock('@/drizzle/schema', () => ({
  contacts: { id: 'id', phone: 'phone' },
}));
...
describe('Chat - convertChatToLead', () => {
  ...
  it('rejects conversion if already converted', async () => {
    mockFindFirst.mockResolvedValue({
      id: 'session-1',
      convertedLeadId: 'existing-contact-1',
    });
