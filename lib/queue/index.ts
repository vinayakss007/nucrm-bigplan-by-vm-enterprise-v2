...
export type JobType = 'send-email' | 'send-notification' | 'send-bulk-emails' | 'export-csv' | 'contact-import' | 'run-automation' | 'send-lead-warming' | 'whatsapp-webhook' | 'webhooks';
...
async function createRedisAdapter(redisUrl: string): Promise<QueueAdapter> {
  ...
  const jobTypes: JobType[] = ['send-email', 'send-notification', 'send-bulk-emails', 'export-csv', 'contact-import', 'run-automation', 'send-lead-warming', 'whatsapp-webhook', 'webhooks'];
