 ...
 *  - The GCM auth tag (16 bytes) is appended by the cipher automatically and
 *    checked by the decipher on final flush.
 *
...
export async function decryptBackupFile(encryptedPath: string, outputPath: string): Promise<void> {
  ...
  // Skip the first 12 bytes (IV) when reading the ciphertext.
  const input = createReadStream(encryptedPath, { start: IV_LENGTH });
