...
| Content-Security-Policy   | ✅ PRESENT | Per-request **nonce-based** CSP from `proxy.ts` (#1070). `script-src 'self' 'nonce-<per-request>'` — `'unsafe-inline'` **removed** from script-src. `style-src 'unsafe-inline'` retained by design (next/font, react-hot-toast, tenant branding). nginx/CDN must NOT set/strip/override this header. |
