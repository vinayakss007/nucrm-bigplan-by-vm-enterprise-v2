...
| **RLS-safe dump**          | `scripts/backup-rls-live.sh`                     | Full dump for managed databases with **no BYPASSRLS role**: sets `app.is_super_admin` + dummy tenant/user GUCs, `pg_dump --schema-only`, sequence values, and client-side `\copy` CSVs per table (pgbouncer-safe). Output: schema, sequences, `data/*.csv.gz`, row-count manifest. Use when `backup-db.sh` can't reach a bypass role (PP-014/#2124). |
| **RLS-safe restore drill** | `scripts/restore-drill.sh <dump dir>`            | Companion verification for the RLS-safe dump: restores schema + data into a scratch DB (same super-admin GUC recipe), re-applies sequences, and asserts every manifest row count. Exit non-zero = the dump is **not** proven restorable. `KEEP=1` retains the scratch DB for inspection. Run weekly (#2124).                                         |
