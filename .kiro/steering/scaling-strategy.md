...
3. **Lazy-load the UI**:
   ...
   export const LazyMyFeature = lazyModule(
     "my-module",
     () => import("@/components/tenant/my-feature"),
   );
   ```
...
Configure hosts in `ecosystem.config.cjs` → `deploy.production.host` array.
...
### Per-Process Limits (ecosystem.config.cjs)
